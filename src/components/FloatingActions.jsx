...
export default function FloatingActions() {
  const { t } = useLanguage();
  const PHONE_NUMBER = process.env.NEXT_PUBLIC_PHONE_NUMBER || '9962548644';
  const WHATSAPP_NUMBER = process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || '9962548644';
  return (
    <div className={styles.floatingContainer}>
      <a href={`tel:${PHONE_NUMBER.replace(/\s+/g, '')}`} className={`${styles.actionBtn} ${styles.callBtn}`} aria-label={t('callUs')}>
        <span className={styles.icon}>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path>
          </svg>
        </span>
        <span className={styles.tooltip}>{t('callUs')}</span>
      </a>
      
      <a href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(`Hello Sri Sankaraa Catering Services! 👋\n\nI would like to inquire about catering services for an upcoming function. Here are our event details:\n\n👤 Name: \n📅 Event Date: \n📍 Venue / Area in Chennai: \n👥 Expected Guests: \n🎉 Occasion (Wedding / Housewarming / Seemantham / Corporate / Birthday): \n\nCould you please share your menu packages and pricing? Looking forward to connecting with you! 🙏`)}`} target="_blank" rel="noopener noreferrer" className={`${styles.actionBtn} ${styles.waBtn}`} aria-label={t('chatWhatsapp')}>
        <span className={styles.icon}>
        ...
        <span className={styles.tooltip}>{t('chatWhatsapp')}</span>
      </a>
    </div>
  );
}
